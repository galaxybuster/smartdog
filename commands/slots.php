<?php
	...
	function slots($str, $sender, $senderId) {
		...
		if ($user->isRegistered) {
			// grab sender money
			$senderMoney = $user->bones;
		} else {
			$response = "i don't think you're registered, i-if that's ok..";
			return $response;
		}
		
		// Check that the gambler has enough to wager
		if ($amount > 0) {
			if (fmod($amount, 1) == 0) {
				if ($senderMoney >= $amount) {
					...
					*/
					...
					$slotValues = array(
						array(":boutit:"=>0.9, ":zippy:"=>3.0, ":wario:"=>1.1, ":eyepop:"=>2, ":mega:"=>3),
						array(":boutit:"=>1.33, ":zippy:"=>0.3, ":wario:"=>0, ":eyepop:"=>0.8, ":mega:"=>0.25));
					/*
					...
					*/
					...
					$response = $slotString . "\r\n" . "You get :bone:" . $payout . " in return";
					return $response;
				} else {
					$response = "you don't have enough, don't gamble your fur away";
					return $response;
				}
			} else {
				$response = "p-please no decimals, i can't understand them...";
				return $response;
			}
		} else {
			$response = "p-please bet a positive amount...";
			return $response;
		}
